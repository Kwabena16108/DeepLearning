...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        # 2) Recurrent layer
        if model_type == "RNN":
            self.recurrent = nn.RNN(emb_size, encoder_hidden_size)
        elif model_type == "LSTM":
            self.recurrent = nn.LSTM(emb_size, encoder_hidden_size)
        else:
            raise ValueError("Invalid model type. Allowed options are 'RNN' (default) and 'LSTM'")
