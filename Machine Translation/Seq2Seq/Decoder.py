...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN"):
        ...
        # 2) Recurrent layer
        if model_type == "RNN":
            self.recurrent = nn.RNN(emb_size, decoder_hidden_size, 1)
        elif model_type == "LSTM":
            self.recurrent = nn.LSTM(emb_size, decoder_hidden_size, 1)
        else:
            raise ValueError(
                "Invalid model type. Allowed options are 'RNN' (default) and 'LSTM'")

        # 3) linear layer with a (log)softmax layer for output
    ...
    def compute_attention(self, hidden, encoder_outputs):
        ...
        #                              BEGIN YOUR CODE                              #
        #############################################################################
        _, N, dims = hidden.size()

        q = hidden.transpose(0, 1)  # [N, 1, H]
        k = encoder_outputs  # [N, T, H]

        # calculate the dot product of `q` and `k` over the last dimension
        # (1, H) x (H, T) = (1, T)
        dot_product = torch.bmm(q, k.transpose(1, 2))  # per batch -> final shape (N, 1, T)
        dot_product = dot_product.squeeze(1)  # reshape to (N, T) by removing the single-value dimension

        # calculate the norm of `q` and `k` over the last dimension
        q_norm = q.norm(dim=-1)
        k_norm = k.norm(dim=-1)
        # calculate the cosine similarity between `q` and `k`
        attn_scores = dot_product / (q_norm * k_norm)
        attn_weights = torch.softmax(attn_scores, dim=-1)
        attention = attn_weights.unsqueeze(1)
    ...
    def forward(self, input, hidden, encoder_outputs=None, attention=False):
        ...
        #       returning it.                                                       #
        #############################################################################
        input = input.transpose(0, 1)  # [1, batch size]
        # [1, batch size, emb size]
        ...
        if attention:
            ...
            hidden = torch.bmm(attn_weights, encoder_outputs).transpose(0, 1)
        hidden = hidden if cell is None else (hidden, cell)
        output, hidden = self.recurrent(embedded, hidden)

        output = self.log_softmax(self.linear(output)).squeeze(0)
