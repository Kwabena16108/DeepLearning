...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        outputs = torch.zeros(out_seq_len, batch_size, vocab_size).to(self.device)

        # 1) Get the last hidden representation from the encoder.
        # encoder_outputs [batch, seq, encoder hidden size]
        # hidden [1, batch, decoder hidden size]
        encoder_outputs, hidden = self.encoder(source)

        # 2) The first input for the decoder should be the <sos> token
        decoder_input = source[:, 0].unsqueeze(1)  # [batch, 1] first in the source sequence
        for t in range(out_seq_len):
            # 3) Feed the first input and hidden state into the decoder one step at a time
            # output shape: [batch, out]
            output, hidden = self.decoder(decoder_input, hidden, encoder_outputs, attention=self.attention)
            # add the output to final outputs
            outputs[t] = output
            # 4) update the input
